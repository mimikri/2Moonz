...
function ShowQuickEditorPage(): void
{
	...
	switch($edit)
	{
		...
		case 'player':
			...
			{
				$tech[]	= ['type'	=> $resource[$ID], 'name'	=> $LNG['tech'][$ID], 'count'	=> pretty_number($UserData[$resource[$ID]]), 'input'	=> $UserData[$resource[$ID]]];
			}
			foreach($reslist['officier'] as $ID)
			{
				$officier[]	= ['type'	=> $resource[$ID], 'name'	=> $LNG['tech'][$ID], 'count'	=> pretty_number($UserData[$resource[$ID]]), 'input'	=> $UserData[$resource[$ID]]];
			}

			$template	= new template();
			$template->assign_vars(['tech'			=> $tech, 'officier'		=> $officier, 'id'			=> $id, 'planetid'		=> $UserData['id_planet'], 'planetname'	=> $PlanetInfo['name'], 'name'			=> $UserData['username'], 'galaxy'		=> $UserData['galaxy'], 'system'		=> $UserData['system'], 'planet'		=> $UserData['planet'], 'authlevel'		=> $UserData['authlevel'], 'authattack'	=> $UserData['authattack'], 'multi'			=> $GLOBALS['DATABASE']->getFirstCell("SELECT COUNT(*) FROM ".MULTI." WHERE userID = ".$id.";"), 'ChangePW'		=> $ChangePW, 'yesorno'		=> [1 => $LNG['one_is_yes_1'], 0 => $LNG['one_is_yes_0']], 'darkmatter'	=> floatToString($UserData['darkmatter']), 'darkmatter_c'	=> pretty_number($UserData['darkmatter'])]);
