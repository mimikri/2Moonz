...
function ShowLogOverview(): void {
	global $LNG;
	$template	= new template();	
	$template->show("LogOverview.tpl");
}

function ShowLogDetail(): void {
	global $LNG;
	$logid = HTTP::_GP('id', 0);
	...
	foreach ($conf_before as $key => $val) {
		if ($key != 'universe') {
			...
			$LogArray[]	= ['Element'	=> $Element, 'old'		=> ($Element == 'urlaubs_until' ? _date($LNG['php_tdformat'], $val) : (is_numeric($val) ? pretty_number($val) : $val)), 'new'		=> ($Element == 'urlaubs_until' ? _date($LNG['php_tdformat'], $conf_after[$key]) : (is_numeric($conf_after[$key]) ? pretty_number($conf_after[$key]) : $conf_after[$key]))];
		}
	}
		
	$template	= new template();	
	$template->assign_vars(['LogArray'		=> $LogArray, 'admin'			=> $result['admin_username'], 'target'		=> $result['universe'], 'id'			=> $result['id'], 'time'			=> _date($LNG['php_tdformat'], $result['time'], $USER['timezone']), 'log_info'		=> $LNG['log_info'], 'log_admin'		=> $LNG['log_admin'], 'log_time'		=> $LNG['log_time'], 'log_target'	=> $LNG['log_universe'], 'log_id'		=> $LNG['log_id'], 'log_element'	=> $LNG['log_element'], 'log_old'		=> $LNG['log_old'], 'log_new'		=> $LNG['log_new']]);
	
	$template->show("LogDetail.tpl");
}

function ShowLogSettingsList(): void {
	global $LNG;
	...
	$targetkey = [0 => $LNG['log_ssettings'], 1 => $LNG['log_usettings'], 2 => $LNG['log_statsettings'], 3 => $LNG['log_chatsettings'], 4 => $LNG['log_tssettings'], 5 => $LNG['log_disclamersettings']];
	
	while ($LogRow = $GLOBALS['DATABASE']->fetch_array($result))
	{			
		$LogArray[]	= ['id'			=> $LogRow['id'], 'admin'			=> $LogRow['admin_username'], 'target_uni'	=> ($LogRow['target'] == 0 ? '' : $LogRow['universe']), 'target'		=> $targetkey[$LogRow['target']], 'time'			=> _date($LNG['php_tdformat'], $LogRow['time'], $USER['timezone'])];
	}
	$GLOBALS['DATABASE']->free_result($result);
	$template->assign_vars(['LogArray'				=> $LogArray, 'log_log'		=> $LNG['log_log'], 'log_admin'		=> $LNG['log_admin'], 'log_time'		=> $LNG['log_time'], 'log_uni'		=> $LNG['log_uni_short'], 'log_target'	=> $LNG['log_target_universe'], 'log_id'		=> $LNG['log_id'], 'log_view'		=> $LNG['log_view']]);
	$template->show("LogList.tpl");
}

function ShowLogPlanetsList(): void {
	global $LNG;
	...
	while ($LogRow = $GLOBALS['DATABASE']->fetch_array($result))
	{			
		$LogArray[]	= ['id'		=> $LogRow['id'], 'admin'		=> $LogRow['admin_username'], 'target_uni'=> $LogRow['universe'], 'target'	=> '['.$LogRow['target_galaxy'].':'.$LogRow['target_system'].':'.$LogRow['target_planet'].'] -> '.$LogRow['target_username'], 'time'		=> _date($LNG['php_tdformat'], $LogRow['time'], $USER['timezone'])];
	}
	$GLOBALS['DATABASE']->free_result($result);
	$template	= new template();	
	$template->assign_vars(['LogArray'		=> $LogArray, 'log_log'		=> $LNG['log_log'], 'log_admin'		=> $LNG['log_admin'], 'log_time'		=> $LNG['log_time'], 'log_uni'		=> $LNG['log_uni_short'], 'log_target'	=> $LNG['log_target_planet'], 'log_id'		=> $LNG['log_id'], 'log_view'		=> $LNG['log_view']]);
	$template->show("LogList.tpl");
}

function ShowLogPlayersList(): void {
	global $LNG;
	...
	while ($LogRow = $GLOBALS['DATABASE']->fetch_array($result))
	{			
		$LogArray[]	= ['id'		=> $LogRow['id'], 'admin'		=> $LogRow['admin_username'], 'target_uni'=> $LogRow['universe'], 'target'	=> $LogRow['target_username'], 'time'		=> _date($LNG['php_tdformat'], $LogRow['time'], $USER['timezone'])];
	}
	$GLOBALS['DATABASE']->free_result($result);
	$template->assign_vars(['LogArray'		=> $LogArray, 'log_log'		=> $LNG['log_log'], 'log_admin'		=> $LNG['log_admin'], 'log_time'		=> $LNG['log_time'], 'log_uni'		=> $LNG['log_uni_short'], 'log_target'	=> $LNG['log_target_user'], 'log_id'		=> $LNG['log_id'], 'log_view'		=> $LNG['log_view']]);
	$template->show("LogList.tpl");
}

function ShowLogPresent(): void {
	global $LNG;
	...
	while ($LogRow = $GLOBALS['DATABASE']->fetch_array($result))
	{			
		$LogArray[]	= ['id'		=> $LogRow['id'], 'admin'		=> $LogRow['admin_username'], 'target_uni'=> $LogRow['universe'], 'target'	=> $LNG['fcm_universe'], 'time'		=> _date($LNG['php_tdformat'], $LogRow['time'], $USER['timezone'])];
	}
	$GLOBALS['DATABASE']->free_result($result);
	$template->assign_vars(['LogArray'		=> $LogArray, 'log_log'		=> $LNG['log_log'], 'log_admin'		=> $LNG['log_admin'], 'log_time'		=> $LNG['log_time'], 'log_uni'		=> $LNG['log_uni_short'], 'log_target'	=> $LNG['log_target_user'], 'log_id'		=> $LNG['log_id'], 'log_view'		=> $LNG['log_view']]);
	$template->show("LogList.tpl");
