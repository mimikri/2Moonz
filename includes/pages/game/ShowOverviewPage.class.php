...
class ShowOverviewPage extends AbstractGamePage
{
	...
	private function GetTeamspeakData(): false|array
	{
		global $USER, $LNG;
		...
		if(empty($tsInfo))
		{
			return ['error'	=> $LNG['ov_teamspeak_not_online']];
		}
		...
		return ['url'		=> sprintf($url, $config->ts_server, $config->ts_tcpport, $USER['username'], $tsInfo['password']), 'current'	=> $tsInfo['current'], 'max'		=> $tsInfo['maxuser'], 'error'		=> false];
	}
	...
	function savePlanetAction(): void
	{
		...
		if (!empty($password))
		{
            ...
            $IfFleets = $db->selectSingle($sql, [':userID'   => $USER['id'], ':planetID' => $PLANET['id'], ':lunaID'   => $PLANET['id_luna']], 'state');
			...
			else
			{
				if($PLANET['planet_type'] == 1) {
                    ...
                    $db->update($sql, [':time'   => TIMESTAMP + 86400, ':planetID' => $PLANET['id']]);
	...
	function show(): void
	{
		...
		{		
			...
			$AllPlanets[] = ['id'	=> $CPLANET['id'], 'name'	=> $CPLANET['name'], 'image'	=> $CPLANET['image'], 'build'	=> $BuildPlanet];
		}
		...
		if ($PLANET['b_building'] - TIMESTAMP > 0) {
			...
			$buildInfo['buildings']	= ['id'		=> $Queue[0][0], 'level'		=> $Queue[0][1], 'timeleft'	=> $PLANET['b_building'] - TIMESTAMP, 'time'		=> $PLANET['b_building'], 'starttime'	=> pretty_time($PLANET['b_building'] - TIMESTAMP)];
		}
		else {
		...
		if (!empty($PLANET['b_hangar_id'])) {
			...
			$buildInfo['fleet']	= ['id'		=> $Queue[0][0], 'level'		=> $Queue[0][1], 'timeleft'	=> $time - $PLANET['b_hangar'], 'time'		=> $time, 'starttime'	=> pretty_time($time - $PLANET['b_hangar'])];
		}
		else {
		...
		if ($USER['b_tech'] - TIMESTAMP > 0) {
			...
			$buildInfo['tech']	= ['id'		=> $Queue[0][0], 'level'		=> $Queue[0][1], 'timeleft'	=> $USER['b_tech'] - TIMESTAMP, 'time'		=> $USER['b_tech'], 'starttime'	=> pretty_time($USER['b_tech'] - TIMESTAMP)];
		}
		else {
        ...
        $onlineAdmins = $db->select($sql, [':universe'     => Universe::current(), ':onlinetime'   => TIMESTAMP-10*60, ':authlevel'    => AUTH_USR]);
        ...
        if($config->ref_active)
		{
			foreach ($RefLinksRAW as $RefRow) {
				$RefLinks[$RefRow['id']]	= ['username'	=> $RefRow['username'], 'points'	=> min($RefRow['total_points'], $config->ref_minpoints)];
			}
		}
		...
		$statData	= Database::get()->selectSingle($sql, [':userId'	=> $USER['id'], ':statType'	=> 1]);

		if(!isset($statData['total_rank']) || $statData['total_rank'] == 0) {
			$rankInfo	= "-";
		} else {
	...
	function actions(): void 
	{
		...
		$this->assign(['ov_security_confirm'		=> sprintf($LNG['ov_security_confirm'], $PLANET['name'].' ['.$PLANET['galaxy'].':'.$PLANET['system'].':'.$PLANET['planet'].']')]);
