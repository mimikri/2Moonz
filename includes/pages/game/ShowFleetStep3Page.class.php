...
class ShowFleetStep3Page extends AbstractGamePage
{
	...
	public function show(): void
	{
		...
		{
			$this->printMessage($LNG['fl_error_same_planet'], [['label'	=> $LNG['sys_back'], 'url'	=> 'game.php?page=fleet1']]);
		}
		...
		if ($targetGalaxy < 1 || $targetGalaxy > $config->max_galaxy || 
			...
			$this->printMessage($LNG['fl_invalid_target'], [['label'	=> $LNG['sys_back'], 'url'	=> 'game.php?page=fleet1']]);
		}

		if ($targetMission == 3 && $TransportMetal + $TransportCrystal + $TransportDeuterium < 1)
		{
			$this->printMessage($LNG['fl_no_noresource'], [['label'	=> $LNG['sys_back'], 'url'	=> 'game.php?page=fleet2']]);
		}
		...
		{
			$this->printMessage($LNG['fl_no_slots'], [['label'	=> $LNG['sys_back'], 'url'	=> 'game.php?page=fleetTable']]);
		}
        ...
        if(!empty($fleetGroup))
		{
            ...
            $ACSTime = $db->selectSingle($sql, [':acsID'        => $fleetGroup, ':maxFleets'    => $config->max_fleets_per_acs], 'ankunft');
        ...
        $targetPlanetData = $db->selectSingle($sql, [':universe'     => Universe::current(), ':targetGalaxy' => $targetGalaxy, ':targetSystem' => $targetSystem, ':targetPlanet' => $targetPlanet, ':targetType' => ($targetType == 2 ? 1 : $targetType)]);

		if ($targetMission == 7)
		{
			if (!empty($targetPlanetData)) {
				$this->printMessage($LNG['fl_target_exists'], [['label'	=> $LNG['sys_back'], 'url'	=> 'game.php?page=fleet1']]);
			}
			
			if ($targetType != 1) {
				$this->printMessage($LNG['fl_only_planets_colonizable'], [['label'	=> $LNG['sys_back'], 'url'	=> 'game.php?page=fleet1']]);
			}
		}
		...
		else
		{
			if ($targetPlanetData["destruyed"] != 0) {
				$this->printMessage($LNG['fl_no_target'], [['label'	=> $LNG['sys_back'], 'url'	=> 'game.php?page=fleet1']]);
			}
		...
		foreach ($fleetArray as $Ship => $Count)
		{
			if ($Count > $PLANET[$resource[$Ship]]) {
				$this->printMessage($LNG['fl_not_all_ship_avalible'], [['label'	=> $LNG['sys_back'], 'url'	=> 'game.php?page=fleetTable']]);
			}
		}
		...
		if ($targetMission == 11)
		{
			...
			if ($activeExpedition >= $maxExpedition) {
				$this->printMessage($LNG['fl_no_expedition_slot'], [['label'	=> $LNG['sys_back'], 'url'	=> 'game.php?page=fleetTable']]);
			}
		}
		...
		if($targetMission == 7 || $targetMission == 15) {
			$targetPlayerData	= ['id'				=> 0, 'onlinetime'		=> TIMESTAMP, 'ally_id'			=> 0, 'urlaubs_modus'		=> 0, 'authattack'		=> 0, 'total_points'		=> 0];
		} elseif($myPlanet) {
		...
		{
			$this->printMessage($LNG['fl_empty_target'], [['label'	=> $LNG['sys_back'], 'url'	=> 'game.php?page=fleet1']]);
		}
		...
		if (!in_array($targetMission, $availableMissions['MissionSelector'])) {
			$this->printMessage($LNG['fl_invalid_mission'], [['label'	=> $LNG['sys_back'], 'url'	=> 'game.php?page=fleet2']]);
		}
		...
		if($targetMission == 1 || $targetMission == 2 || $targetMission == 9) {
			...
			{
				$this->printMessage($LNG['fl_bash_protection'], [['label'	=> $LNG['sys_back'], 'url'	=> 'game.php?page=fleetTable']]);
			}
		}
		...
		if($targetMission == 1 || $targetMission == 2 || $targetMission == 5 || $targetMission == 6 || $targetMission == 9)
		{
			...
			{
				$this->printMessage($LNG['fl_admin_attack'], [['label'	=> $LNG['sys_back'], 'url'	=> 'game.php?page=fleetTable']]);
			}
			...
			{
				$this->printMessage($LNG['fl_player_is_noob'], [['label'	=> $LNG['sys_back'], 'url'	=> 'game.php?page=fleetTable']]);
			}
			
			if ($IsNoobProtec['StrongPlayer']) {
				$this->printMessage($LNG['fl_player_is_strong'], [['label'	=> $LNG['sys_back'], 'url'	=> 'game.php?page=fleetTable']]);
			}
		}
		...
		{	
			if($targetPlayerData['ally_id'] != $USER['ally_id']) {
                ...
                $buddy = $db->selectSingle($sql, [':ownerID'  => $targetPlayerData['id'], ':userID'   => $USER['id']], 'state');

                if($buddy == 0) {
					$this->printMessage($LNG['fl_no_same_alliance'], [['label'	=> $LNG['sys_back'], 'url'	=> 'game.php?page=fleetTable']]);
				}
			}
		}
		...
		if ($PLANET[$resource[903]] < $consumption) {
			$this->printMessage($LNG['fl_not_enough_deuterium'], [['label'	=> $LNG['sys_back'], 'url'	=> 'game.php?page=fleetTable']]);
		}
		...
		if($targetMission == 5 || $targetMission == 11 || $targetMission == 15)
		{
			...
			{
				$this->printMessage($LNG['fl_hold_time_not_exists'], [['label'	=> $LNG['sys_back'], 'url'	=> 'game.php?page=fleetTable']]);
			}
		...
		$fleetResource	= [901	=> min($TransportMetal, floor($PLANET[$resource[901]])), 902	=> min($TransportCrystal, floor($PLANET[$resource[902]])), 903	=> min($TransportDeuterium, floor($PLANET[$resource[903]] - $consumption))];
		
		$StorageNeeded		= array_sum($fleetResource);
		
		if ($StorageNeeded > $fleetStorage)
		{
			$this->printMessage($LNG['fl_not_enough_space'], [['label'	=> $LNG['sys_back'], 'url'	=> 'game.php?page=fleetTable']]);
		}
		...
		$this->assign(['targetMission'		=> $targetMission, 'distance'			=> $distance, 'consumption'		=> $consumption, 'from'				=> $PLANET['galaxy'] .":". $PLANET['system']. ":". $PLANET['planet'], 'destination'		=> $targetGalaxy .":". $targetSystem .":". $targetPlanet, 'fleetStartTime'	=> _date($LNG['php_tdformat'], $fleetStartTime, $USER['timezone']), 'fleetEndTime'		=> _date($LNG['php_tdformat'], $fleetEndTime, $USER['timezone']), 'MaxFleetSpeed'		=> $fleetMaxSpeed, 'FleetList'			=> $fleetArray]);
